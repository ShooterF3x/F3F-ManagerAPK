...
function getCalculatedTargetWeight(w, f, g) {
    ...
    if (active.isDouble && w > (active.vp || 10)) { baseTarget = (active.a2 * w + active.b2) * 1000; } 
    else { baseTarget = (active.a * w + active.b) * 1000; }
...
function getEffectiveStock(g, idx, typeFull) {
    const c = g.chambers[idx]; const stockKey = 'stock_' + typeFull;
...
window.autoFillBallast = function() {
    ...
    function getStats(load) {
        ...
        g.chambers.forEach((c,i) => { let w = load[i].b*(c.mass_brass||0) + load[i].l*(c.mass_lead||0) + load[i].t*(c.mass_tungsten||0); m += w; mom += w * c.dist; });
        return { m, cg: (m > 0 ? mom/m : 0) };
    }

    function getCost(stats) {
        let diffW = stats.m - targetW; let diffCG = stats.cg - g.sessionTargetCG; let wCost = 0;
        if (diffW > activeOpts.wMax) wCost = (diffW - activeOpts.wMax) * 50.0; else if (diffW < -activeOpts.wMin) wCost = Math.abs(diffW + activeOpts.wMin) * 50.0; else wCost = Math.abs(diffW) * 0.1;
        let cgCost = 0;
        if (diffCG > 0) { if(diffCG > activeOpts.cgTolPlus) cgCost = (diffCG - activeOpts.cgTolPlus) * 10000.0; else cgCost = diffCG * 15.0; } 
        else { if(Math.abs(diffCG) > activeOpts.cgTolMinus) cgCost = (Math.abs(diffCG) - activeOpts.cgTolMinus) * 10000.0; else cgCost = Math.abs(diffCG) * 15.0; }
        return wCost + cgCost;
    }

    const getGroupUsage = (grpId, matCode, load) => { let u = 0; g.chambers.forEach((ch, idx) => { if(ch.group == grpId) u += load[idx][matCode]; }); return u; };
    let bestGlobalLoadout = g.loadout.map(() => ({b:0, l:0, t:0})); let bestGlobalScore = Infinity;
            ...
            g.chambers.forEach((c, i) => {
                ...
                for(let k=0; k<numToAdd; k++) {
                    ...
                    ['brass', 'lead', 'tungsten'].forEach(mat => {
                        let mCode = matMap[mat]; let eff = getEffectiveStock(g, i, mat);
                    ...
                    if (availableMats.length > 0) { let m = availableMats[Math.floor(Math.random() * availableMats.length)]; currentLoadout[i][m]++; }
                }
            });
        }

        let currentScore = getCost(getStats(currentLoadout)); let improved = true; let steps = 0;

        while (improved && steps < 500) {
            improved = false; let bestMove = null; let bestMoveScore = currentScore;
            indices.forEach((i) => { 
                let c = g.chambers[i]; let L = currentLoadout[i];
                if((L.b + L.l + L.t) < c.max) {
                    ['brass', 'lead', 'tungsten'].forEach(mat => {
                        let mCode = matMap[mat]; let eff = getEffectiveStock(g, i, mat); let canAdd = false;
                        if(eff.val > 0) { if(eff.isGroup) { if(getGroupUsage(c.group, mCode, currentLoadout) < eff.val) canAdd = true; } else { if(L[mCode] < eff.val) canAdd = true; } }
                        if(canAdd) {
                            let testLoad = JSON.parse(JSON.stringify(currentLoadout)); testLoad[i][mCode]++; let h = getCost(getStats(testLoad));
                ...
                ['brass', 'lead', 'tungsten'].forEach(mat => {
                    ...
                    if(L[mCode] > 0) { let testLoad = JSON.parse(JSON.stringify(currentLoadout)); testLoad[i][mCode]--; let h = getCost(getStats(testLoad)); if(h < bestMoveScore) { bestMoveScore = h; bestMove = {action:'sub', i, mat: mCode}; } }
                });
            });
            if (bestMove) { if (bestMove.action === 'add') currentLoadout[bestMove.i][bestMove.mat]++; if (bestMove.action === 'sub') currentLoadout[bestMove.i][bestMove.mat]--; currentScore = bestMoveScore; improved = true; }
            steps++;
        }
        if (currentScore < bestGlobalScore) { bestGlobalScore = currentScore; bestGlobalLoadout = JSON.parse(JSON.stringify(currentLoadout)); }
    }
    g.loadout = bestGlobalLoadout; save(); 
    if(window.renderCalc) window.renderCalc();
};
